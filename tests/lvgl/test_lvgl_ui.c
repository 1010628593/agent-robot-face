/* Real LVGL 9.4 software renderer + production three-screen code.
 * No ESP-IDF/BSP hardware, display controller, USB, accounts or model services.
 * Each CTest case is a fresh process so file-static UI state cannot leak. */
#ifdef NDEBUG
#undef NDEBUG
#endif
#include "bot_ui.h"
...
static void tick(uint32_t elapsed)
{
    for (uint32_t i=0; i<elapsed; i+=10) {
        clock_ms += 10;
        /* Drive the model explicitly: disable only its automatic demo advance. */
        g_ui.sim_cycle_ms = clock_ms;
...
{
    const char *dir=getenv("BOT_SNAPSHOT_DIR");
    if(!dir)return;
    char path[1024];
    int len=snprintf(path,sizeof(path),"%s/%s.ppm",dir,name);
...
int main(int argc,char **argv)
{
    ...
    if(!strcmp(argv[1],"smoke")) {
        ...
        unsigned lit=0;
        for(int y=140;y<320;y++)for(int x=100;x<366;x++)
            lit += (pixels[y][x]&0xffffff)!=0;
        assert(lit>1000);
        snapshot("face");
        contact=(lv_point_t){233,233};down=true;tick(650);down=false;tick(50);
        ...
        show_stats(BOT_AGENT_WORKBUDDY,1);assert(label(root,"N/A"));
        assert(bars(root)==0);
        show_stats(BOT_AGENT_HERMES,1);assert(label(root,"UNLIMITED"));
        assert(bars(root)==0);
        ...
        show_stats(BOT_AGENT_CODEX,1);
        assert(label(root,"0% LEFT") && !label(root,"OVER"));
