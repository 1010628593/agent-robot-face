...
static void fill_card(lv_obj_t *card, lv_obj_t *init_label, uint8_t agent_idx,
                      bool center)
{
    const bot_ui_agent_view_t *a = &g_ui.agents[agent_idx];
...
void picker_dev_build(lv_obj_t *scr)
{
...
void picker_dev_refresh(void)
{
    ...
    const bot_ui_agent_view_t *a = &g_ui.agents[p];
...
void picker_dev_animate(int direction) {
    bot_ui_preview_slide(s_card_c,direction);
    bot_ui_preview_slide(s_card_l,direction);
    bot_ui_preview_slide(s_card_r,direction);
}
void picker_dev_press(bool pressed) {
    lv_obj_set_style_bg_color(s_card_c,lv_color_hex(BOT_OS_PRESS_COLOR),0);
    bot_ui_press_feedback(s_card_c,pressed);
}
